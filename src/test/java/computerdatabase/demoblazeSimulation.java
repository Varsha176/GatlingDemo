...
public class demoblazeSimulation extends Simulation {
  ...
  ChainBuilder selectItem=exec(
            http("HomePage").options("/bycat").resources(
            http("request_1").post("/bycat").body(RawFileBody("computerdatabase/demoblazesimulation/0001_request.json"))))
          .pause(1).exec(http("request_2").get(uri1 + "/prod.html?idp_=8").resources(http("request_3")
                                    ...
                                    http("request_7")
                                            .options("/view"),
                                    http("request_8")
                                            .post("/view").body(RawFileBody("computerdatabase/demoblazesimulation/0008_request.json"))
                            )
                    );

    ChainBuilder addToCart =  exec(
            http("Add To Cart").options("/addtocart").resources(http("request_10")
            .post("/addtocart").body(RawFileBody("computerdatabase/demoblazesimulation/0010_request.html")))
            ).pause(3).exec(http("request_11").get(uri1 + "/cart.html").resources(http("request_12")
                                    .get(uri1 + "/config.json"),
                            http("request_13")
                                    .get(uri3 + "/index.m3u8"),
                            http("request_14")
    ...
    private ScenarioBuilder users = scenario("demoblazeSimulation").exec(selectItem,addToCart);

    private ScenarioBuilder admin = scenario("demoblazeSimulation1").exec(selectItem,addToCart,deleteItem);
